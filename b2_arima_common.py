...
class B2_rpy_arima(object):
    def __init__(self, configfile):
        ...
        self.TrainingPeriod = int(config.get("ARIMA", "TrainingPeriod"))
        self.MaxBarInterval = int(config.get("ARIMA", "MaxBarInterval"))
        self.PriceFolder    =     config.get("ARIMA", "PriceFolder"   )
        self.CoefFolder     =     config.get("ARIMA", "CoefFolder"    )
        self.ForecastFolder =     config.get("ARIMA", "ForecastFolder")
    ...
    def load_prev_forecasts(self):
        filelist = [ f for f in listdir(self.ForecastFolder) if isfile(join(self.ForecastFolder,f)) ]
        # print filelist
    ...
    def calc_forecast(self, py_ls_date_full, py_ls_ln_avgpx_full, how_many_days_bk, barintvl, barintvlshift):
        ...
        if len(py_ls_ln_avgpx) < self.TrainingPeriod:
            return (None,None)

        py_ls_date = py_ls_date[-self.TrainingPeriod:]
        py_ls_ln_avgpx = py_ls_ln_avgpx[-self.TrainingPeriod:]
        ...
        py_ls_date = (list(reversed(py_ls_date)))[-self.TrainingPeriod:]
        py_ls_ln_avgpx = (list(reversed(py_ls_ln_avgpx)))[-self.TrainingPeriod:]
